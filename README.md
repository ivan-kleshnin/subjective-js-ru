...
Материал рассчитан на программистов / сисадминов, не знающих или плохо знающих JS.
...
В процессе изложения, точность фактов и формулировок приносится в жертву, там где идёт
вразрез с педагогическими целями.
...
JS использует автоприведение типов (автокоэрцию), в случае обнаружения операндов
различных типов:
...
В интернете есть множество примеров и таблиц, описывающих правила автокоэрцию ("js wtf stackoverflow").
...
В JS определено 5 примитивных типов и это множество нерасширяемо:
...
Досадным исключением является `null`, который, вследствие технической ошибки, имеет неверный `typeof`:

```js
typeof null // 'object'
```

Класс с конструктором без аргументов можно инстанциировать без скобочек:<br/>
`new Pepe` эквивалентно `new Pepe()`.
...
Чёткого и логического соответствия между вызовом класса как конструктора и как функции в JS нет:

```js
String(foo)     // приводит foo к типу 'string'               (ок)
new String(foo) // приводит foo к типу 'String'               (не ок)
Date(foo)       // некорректный вызов, игнорирует аргумент    (не ок)
new Date(foo)   // парсит foo и возвращает объект типа `Date` (ок)
```

Повторим совет данный выше:

* для П типов всегда используйте литералы и одноимённые функции: `"foo"` и `String("foo")`
* для О типов (в т.ч. пользовательских) всегда используйте конструкторы с `new`: `new Date("1999-01-01")`

Поскольку множество П типов невелико и не расширяемо – выполнение данного правила не является большой проблемой.
