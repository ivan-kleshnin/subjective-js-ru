...
1. Тип из категории П может быть проверен с помощью ключевого слова `typeof`.
...
2. Тип из категории О может быть проверен с помощью ключевого слова `instanceof`
...
3. Для примитивных типов можно полагаться на автоконверсию в Boolean
...
4. Для объектных типов нельзя полагаться на автоконверсию в Boolean
...
будут выполняться иначе (см. ниже).
